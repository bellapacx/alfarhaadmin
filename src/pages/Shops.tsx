...
export default function Shops({ onLogout }: { onLogout: () => void }) {
  ...
  const fetchShops = async () => {
    try {
      const res = await axios.get("https://alfarhaapp.onrender.com/shops");
      setShops(res.data);
    } catch (err) {
  ...
  const fetchCommissions = async (shopId: string) => {
    try {
      const res = await axios.get(
        `https://alfarhaapp.onrender.com/shop_commissions/${shopId}`
      );
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post("https://alfarhaapp.onrender.com/shops", {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(
        `https://alfarhaapp.onrender.com/shops/${form.shop_id}`,
        updatePayload
      );
  ...
  const handleDelete = async (shopId: string) => {
    ...
    try {
      await axios.delete(`https://alfarhaapp.onrender.com/shops/${shopId}`);
  ...
  const isEditing =
    form.shop_id && shops.some((s) => s.shop_id === form.shop_id);
